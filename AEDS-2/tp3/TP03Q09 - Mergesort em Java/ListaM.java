...
public class ListaM {
    ...
    private int n;
    public ListaM (int maxTam) { // A constante maxTam define o tamanho maximo permitido na lista
    ...
    public void intercalar(int esq, int meio, int dir){
        ...
        //Definir tamanho dos dois subseriess
        n1 = meio-esq+1;
        n2 = dir - meio;
  
        SerieV[] a1 = new SerieV[n1+1];
        SerieV[] a2 = new SerieV[n2+1];
  
        //Inicializar primeiro subseries
        for(i = 0; i < n1; i++){
           a1[i] = series[esq+i];
        }
  
        //Inicializar segundo subseries
        for(j = 0; j < n2; j++){
           a2[j] = series[meio+j+1];
        }
  
        //Sentinela no final dos dois seriess
        a1[i].setEpisodios(0x7FFFFFFF); 
        a2[j].setEpisodios(0x7FFFFFFF);
  
        //Intercalacao propriamente dita
        for(i = j = 0, k = esq; k <= dir; k++){
           series[k] = (a1[i].getEpisodios() <= a2[j].getEpisodios()) ? a1[i++] : a2[j++];
        }
     }

    //  ((a1[i].getEpisodios() <= a2[j].getEpisodios()) || ((series[i].getEpisodios() == (series[j].getEpisodios())) && (series[j].getNome().compareTo(series[j].getNome()) > 0)))
    ...
    public static void main(String[] args) throws Exception {
        ...
        lista.mergesort(0, numEntrada-1);
    }
}