...
class SerieV {
    ...
    public int getDoidao(){
        return (episodios * 1000) + temporadas; 
    }
...
public class ListaR {
    ...
    private int n;
    public ListaR (int maxTam) { // A constante maxTam define o tamanho maximo permitido na lista
    ...
    public int getMaior() {
        SerieV maior = series[0];
 
         for (int i = 0; i < n; i++) {
          if(maior.getDoidao() < series[i].getDoidao()){
             maior = series[i];
          }
         }
        return maior.getDoidao();	
     }

    public void countingsort() {
        //series para contar o numero de ocorrencias de cada elemento
        int[] count = new int[getMaior() + 1];
        SerieV[] ordenado = new SerieV[n];
  
        //Inicializar cada posicao do series de contagem 
        for (int i = 0; i < count.length; count[i] = 0, i++);
  
        //Agora, o count[i] contem o numero de elemento iguais a i
        for (int i = 0; i < n; count[series[i].getDoidao()]++, i++);
  
        //Agora, o count[i] contem o numero de elemento menores ou iguais a i
        for(int i = 1; i < count.length; count[i] += count[i-1], i++);
  
        //Ordenando
        for(int i = n-1; i >= 0; ordenado[count[series[i].getDoidao()]-1] = series[i], count[series[i].getDoidao()]--, i--);
